...
public class TwoDice {
    ...
    private float cubeHalfSize = 0.24f;

    // Constructor - Set up the vertex buffer
    public TwoDice(Context context) {
    ...
    public void drawrandom(GL10 gl){
        int r = (int) (Math.random() * (5 - 0)) + 0;
        Log.d("sunil","random number:-"+r);
        gl.glFrontFace(GL10.GL_CCW);
        ...
        gl.glPushMatrix();
        gl.glTranslatef(0f, 0f, cubeHalfSize);
        gl.glBindTexture(GL10.GL_TEXTURE_2D, textureIDs[r]);
        gl.glDrawArrays(GL10.GL_TRIANGLE_STRIP, 0, 4);
        gl.glPopMatrix();
